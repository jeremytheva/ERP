...
function isTaskActive(task: UiTask, round: number) {
  const startRound = task.startRound ?? task.round ?? 1;
  if (round < startRound) {
    return false;
  }

  const recurrence = task.roundRecurrence ?? "Once";
  if (recurrence === "Continuous" || recurrence === "RoundStart") {
...
export default function MainScreen() {
  ...
  const allRoles = useMemo<RoleName[]>(() => {
    const unique = Array.from(new Set(baseTasks.map((task) => task.role).filter(Boolean)));
  ...
  const visibleTasks = useMemo(() => {
    ...
    return baseTasks
      .filter((task) => {
        const roleMatch = effectiveRoles.includes(task.role);
        const roundMatch = isTaskActive(task, currentRound);
